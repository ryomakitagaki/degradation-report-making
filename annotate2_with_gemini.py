...
特定したひび割れの上に、鮮明な赤色(透過率80%)の線（太さ2-3ピクセル）を描画した画像を生成してください。
...
建築物の表面に欠損や剥がれを特定した場合，
特定したすべて欠損や剥がれを、鮮明な赤色で塗りつぶした画像を生成してください。
直線的なタイルやパネル外壁の目地や建材の稜線には赤線を引かないでください。
元の画像と赤塗領域のみで構成された画像を返してください。
"""


# --- プロンプトの設定 上から選ぶ---
PROMPT_FOR_NANOBANANA = PROMPT_FOR_NANOBANANA_V5
